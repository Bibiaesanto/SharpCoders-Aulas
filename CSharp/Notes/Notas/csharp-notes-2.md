...
- reaproveitamento (reuso, e se o código fosse muito grande)
- instrução isolada/ divisão de tarefas
- utilidade
- modularização/ bloco de código
...
- tento retirnar antes e mantenho um caso padrão
...
static void Sacar(double quantia){
    ...
    if(saldo < quantia)
    throw new Exception("Sando insuficiente");
